"""Unit tests for src/data/orats_core_client.py.
...
def test_success_returns_history_frame(_token_env):
    client, session = _client(
        [
            FakeResponse(
                ...
                _payload(
                    [
                        {"ticker": "AAPL", "tradeDate": "2024-01-02", "assetType": 0},
                        {"ticker": "AAPL", "tradeDate": "2024-01-03", "assetType": 0},
                    ]
                ),
            )
        ]
    )
    df = client.fetch_asset_type_history("AAPL")
    assert list(df.columns) == ["ticker", "tradeDate", "assetType"]
    assert len(df) == 2
    assert session.calls[0]["params"]["ticker"] == "AAPL"


def test_empty_data_is_successful_empty_not_error(_token_env):
    client, _ = _client([FakeResponse(200, _payload([]))])
    df = client.fetch_asset_type_history("NOPE")
    assert df.empty
    assert list(df.columns) == ["ticker", "tradeDate", "assetType"]
...
def test_missing_data_key_is_failure(_token_env):
    ...
    with pytest.raises(OratsCoreError, match="missing the 'data' key"):
        client.fetch_asset_type_history("AAPL")
...
def test_retryable_then_success(_token_env):
    client, session = _client(
        [
            FakeResponse(429, headers={"Retry-After": "0"}),
            requests.ConnectionError("transient"),
            FakeResponse(
                200, _payload([{"ticker": "AAPL", "tradeDate": "2024-01-02", "assetType": 1}])
            ),
        ]
    )
    df = client.fetch_asset_type_history("AAPL")
...
def test_token_never_appears_in_logs_or_errors(_token_env, caplog):
    ...
    # Retryable + transport error whose message embeds the token, then a
    # non-retryable error whose body embeds the token.
    client, _ = _client(
        [
